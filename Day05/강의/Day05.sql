1. 인덱스(INDEX)
    ...
    - 일반적으로 인덱스가 생성되지 않은 테이블의 데이터 저장은
      저장순서와 무관하게 저장
    - 인덱스 생성되어 있지 않은 테이블의 검색은 풀 스캔 검색으로 진행
      모든 테이블의 데이터 비교하여 검색
    - 테이블에 인덱스를 생성하게 되면 인덱스로 지정된 컬럼만 
      따로 시스템 테이블로 생성되어 관리됨
...
3. 인덱스의 조건
    ...
    - 검색하는 데이터 양이 전체 데이터의 5%미만 일때 지정
      (전체)10,000,000건 -> 200,000건 검색할 때
    - FK 컬럼은 반드시 인덱스를 생성
    - 조건절로 검색 할 때 인덱스가 지정된 컬럼은 검색속도가 상승
4. 인덱스의 생성
    - 자동 생성
...
8. 뷰의 생성
    ...
    (WITH READ ONLY CONSTRAINT 제약조건명);
      ...
      WITH CHECK OPTION : 뷰에 의해서 검색 가능한 행만
                          DML 구문의 사용이 가능하도록 설정
      WITH READ ONLY : 뷰를 통해서는 SELECT 구문만 사용 가능하도록 설정
9. 인라인 뷰
    - FROM 구문에 선언된 서브쿼리를 인라인 뷰라고 함
    - 인라인 뷰는 뷰가 따로 생성되지 않고 조회 구문이 실행될 때만 존재
    - 조인이나 조검 검색을 할 때 자주 사용
    - 그룹 함수를 써야될 때 주로 사용
10. ROWNUM
...
11. 시퀀스(SEQUENCE)
    ...
    - 시퀀스는 게시물번호와 같이 절대 중복되서는 안되는 컬럼에 지정
12. 시퀀스의 생성
    ...
    - 자동으로 유일한 값을 생성하는 시퀀스를 만듬
    - START WITH : 할당될 값의 시작 값을 지정
    - INCREMENT BY : 생성되는 값의 증가 값을 지정
    - MAXVALUE : 상한 값 지정. 기본 값은 NOMAXVALUE
    - MINVALUE : 하한 값 지정. 기본 값은 NOMINVALUE
    - CYCLE : 상한 값까지 생성 후 다시 하한 값부터 재생성될지 여부 지정
              기본 값 NOCYCLE
              NOCYCLE로 지정 시 상한 값까지 가면 더 이상 값이 생성되지 않음
    - CACHE : 캐시메모리 생성개수 지정
              기본 값은 NOCACHE
...
15. 자연 조인(NATURAL JOIN)
    - 자연 조인은 동일한 타입과 이름을 가진 컬럼을 
      조인 조건으로 이용하는 조인 문장
    - 등가 조인에서만 사용가능
    - 조인되는 컬럼명의 명시없이 사용 가능
    - SELECT 컬럼1
           , 컬럼2
           , 컬럼3
           ....
        FROM 테이블 A
        NATURAL JOIN 조인될 테이블 B
        WHERE 검색 조건(조인조건은 사용불가);
    - 자연 조인에서는 두 테이블간 공통된 컬럼이 반드시 하나만 존재해야됨
16. USING 구문 조인
    - 공통되는 컬럼이 두 개이상이거나 조인되는 컬럼명을 명시하고 싶을 때
      USING 구문 사용하여 조인
    - SELECT 컬럼1
           , 컬럼2
           ...
        FROM 테이블 A
        JOIN 조인될 테이블 B USING (조인될 컬럼명1)
        JOIN 조인될 테이블 C USING (조인될 컬럼명2)
        ...
        WHERE 조회조건;
17. 다중 열의 IN 연산자
    - SELECT 컬럼1
