...
12. 변환 함수
    ...
    - TO_CHAR에 날짜 입력되거나 TO_DATE의 출력형식
      ...
      MON : 월을 한글이나 영문의 약자로 표시(한글은 MONTH와 동일)
            MAR, 3월....
      DD : 일을 두자리로 표시
      DAY : 일을 영문이나 한글 요일로 표시
            MONDAY, SUNDAY, 월요일, 일요일....
      DY : 일을 영문의 약자나 한글의 약자로 표시
           MON, SUN, 일, 월....
      HH24 : 시간을 1 ~ 24까지 표시
      HH : 시간을 1 ~ 12까지 표시
           정확한 표시를 위해 AM/PM을 추가하는 게 권장됨
      MI : 분을 두자리로 표현
      SS : 초를 두자리로 표현
...
13. 그룹 함수
    ...
    - GROUP BY 주의점
      SELECT 구문에 포함된 컬럼들은 모두 GROUP BY로 묶어야 됨
14. 그룹 함수와 HAVING 구문
    - GROUP BY 구문에 조건 추가할 때는 HAVING 구문 사용
    - HAVING 구문은 GROUP BY에 의해 그룹화된 걸럼의 조건만 지정가능
...
16. DML(Data Manipulation Language)
    - 데이터를 입력, 수정, 삭제하는 명령어을 뜻함
    - 오라클에서는 트랜잭션이라는 명칭으로 일련의 작업들을 관리
    - 트랜잭션은 작업의 단위
